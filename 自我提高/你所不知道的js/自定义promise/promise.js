...
(function(window) {
    ...
    Promise.prototype.then = function(onResolved,onRejected)  {
        ...
        const that = this;
        //返回一个promise 新的promise状态的由会滴函数的执行结果决定
        return new Promise((resolve,reject) => {

            //调用指定的回调函数处理，并影响返回新的promise的结果
            function handler(callback) {
                 /**
                 * 由于新的promise的结果有回调函数的执行结构决定 分为三种状态
            ...
            if(that.status == PENDING) {    //当前的状态为pending状态 时存储成功和失败的回调函数
                that.callBacks.push({       //如果直接onResolved 是没法改变新promise的状态的 而我们定义的handler函数恰巧具备连个功能执行回调和改变新promise的状态
                    onResolved() {
                        handler(onResolved)
                    },
                    onRejected() {
